...
func newTCPGROTable() *tcpGROTable {
	t := &tcpGROTable{
		itemsByFlow: make(map[tcpFlowKey][]tcpGROItem, tunIdealBatchSize),
		itemsPool:   make([][]tcpGROItem, tunIdealBatchSize),
	}
	for i := range t.itemsPool {
		t.itemsPool[i] = make([]tcpGROItem, 0, tunIdealBatchSize)
	}
	return t
}
...
func newUDPGROTable() *udpGROTable {
	u := &udpGROTable{
		itemsByFlow: make(map[udpFlowKey][]udpGROItem, tunIdealBatchSize),
		itemsPool:   make([][]udpGROItem, tunIdealBatchSize),
	}
	for i := range u.itemsPool {
		u.itemsPool[i] = make([]udpGROItem, 0, tunIdealBatchSize)
	}
	return u
}
