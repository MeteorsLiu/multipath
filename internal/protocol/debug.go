...
func debugFrame(frame Frame) string {
	...
	case BandwidthProbeBody:
		return fmt.Sprintf("%s train_id=%d probe_id=%d seq=%d count=%d send_ms=%d target_bps=%d train_remaining=%d payload_len=%d", base, body.TrainID, body.ProbeID, body.Seq, body.Count, body.SendMS, body.TargetBps, body.TrainBytesRemaining, len(body.Payload))
