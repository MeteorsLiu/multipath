...
// defaultDataIDDedupeWindow is the number of recent DATA identities tracked behind
...
const defaultDataIDDedupeWindow = 1 << 16

// dedupeBehindThreshold splits "ahead of the window" from "behind / already
// evicted" when interpreting a wrapped uint32 DATA-id delta.
const dedupeBehindThreshold = uint32(1) << 31

// dataIDDedupe tracks one lane's DATA identities in a sliding window
// [base, base+span). It is separate from the FEC receive windows.
type dataIDDedupe struct {
	set    *bitset.BitSet
	span   uint32
	base   uint32
	inited bool
...
func newDataIDDedupe(span uint32) *dataIDDedupe {
	if span == 0 {
		span = defaultDataIDDedupeWindow
	}
	return &dataIDDedupe{
...
// mark records dataID and reports whether this is the first time it has been
// seen in the active window. It returns false for a duplicate or an id older
// than the tracking window.
func (d *dataIDDedupe) mark(dataID uint32) bool {
	if !d.inited {
		...
		d.base = dataID - (d.span - 1)
		d.inited = true
	}

	delta := dataID - d.base // uint32 wraparound is intentional
	switch {
	case delta < d.span:
		// Within the current window.
	case delta >= dedupeBehindThreshold:
		// dataID sorts before base: older than the window, assume emitted.
		return false
	default:
		// dataID is ahead of the window; slide base forward so it becomes
		// the newest tracked id, evicting the oldest.
		newBase := dataID - d.span + 1
		d.evict(newBase)
		d.base = newBase
	}

	pos := uint(dataID % d.span)
	if d.set.Test(pos) {
		return false
	}
	d.set.Set(pos)
...
func (d *dataIDDedupe) seen(dataID uint32) bool {
	if d == nil || !d.inited {
		return false
	}
	delta := dataID - d.base // uint32 wraparound is intentional
	...
	return d.set.Test(uint(dataID % d.span))
}

// evict clears the bits for packet ids in [base, newBase) as the window slides
// forward to newBase.
func (d *dataIDDedupe) evict(newBase uint32) {
