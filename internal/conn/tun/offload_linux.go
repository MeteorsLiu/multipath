...
func newTCPGROTable() *tcpGROTable {
	t := &tcpGROTable{
		itemsByFlow: make(map[tcpFlowKey][]tcpGROItem, 1024),
		itemsPool:   make([][]tcpGROItem, 1024),
	}
	for i := range t.itemsPool {
		t.itemsPool[i] = make([]tcpGROItem, 0, 1024)
	}
	return t
}
...
func newUDPGROTable() *udpGROTable {
	u := &udpGROTable{
		itemsByFlow: make(map[udpFlowKey][]udpGROItem, 1024),
		itemsPool:   make([][]udpGROItem, 1024),
	}
	for i := range u.itemsPool {
		u.itemsPool[i] = make([]udpGROItem, 0, 1024)
	}
	return u
}
...
func gsoSplit(in []byte, hdr virtioNetHdr, outBuffs [][]byte, sizes []int, outOffset int, isV6 bool) (int, error) {
	...
	for ; nextSegmentDataAt < len(in); i++ {
		if i == len(outBuffs) {
			return i - 1, fmt.Errorf("too many segments")
		}
