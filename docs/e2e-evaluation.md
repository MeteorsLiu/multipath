...
| LINK_STATUS backlogged QoS | one UDP-first lane with `fec=true` and warm TCP shadow | client DATA is first moved to TCP, then TCP tunnel traffic is delayed while UDP shadow stays clean | server receive-side FEC differential QoS emits TCP `backlogged`, client applies it, and client DATA returns to UDP |
