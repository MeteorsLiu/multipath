...
| Leg selector | one UDP-first lane with TCP fallback over one veth path | 30% UDP tunnel loss (degraded but not dead) | the leg selector sends DATA frames over TCP even though UDP remains active, and returns to UDP after the loss clears |
